...
def findallIntlCourtMatches(findAllInstances,text,search_terms,term_idx=0):
    '''
...
def getIntlCourtData(text,regex_df,regex_list,file,country_name=None,year=None):
    '''
    Inputs:
    ...
    '''
    regex_intl_ct_results = findallIntlCourtMatches(helpers.findAllInstances,text,regex_list,0)
...
def insertIntlCourtData(country_name,year,file,regex_df,regex_table,mysql_table,connection_info):
    '''
    Inputs:
    ...
    try:
        courtData = getIntlCourtData(text=fileText,country_name=country_name,year=year,regex_df=regex_df,regex_list=regex_table,file=file)
